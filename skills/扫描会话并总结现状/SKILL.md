...
根据用户提供的会话 id，找到当前运行环境规定的会话文件，从主会话到子 agent 会话完整读取，并整理该会话中 agent 工作的最终/最新状态。
...
- 如果主会话创建了子 agent，必须读取所有能定位到的子 agent 会话文件；找不到的子 agent 要在整理中说明“未找到对应会话文件，只能依据主会话通知”。
...
脚本不会替 agent 下结论；最终整理必须由 agent 阅读脚本输出后归并。
